...
def send_api_request(prompt, max_tokens=2000, language='pl'):
    ...
    language_prompts = {
        'pl':
        "Jesteś ekspertem w optymalizacji CV i doradcą kariery. ZAWSZE odpowiadaj w języku polskim, niezależnie od języka CV lub opisu pracy. Używaj polskiej terminologii HR i poprawnej polszczyzny.",
        'en':
        "You are an expert resume editor and career advisor. ALWAYS respond in English, regardless of the language of the CV or job description. Use proper English HR terminology and grammar."
    }

    system_prompt = DEEP_REASONING_PROMPT + "\n" + language_prompts.get(
        language, language_prompts['pl'])
    ...
    try:
        ...
        logger.debug(
            f"API Key present: {'Yes' if OPENROUTER_API_KEY else 'No'}")
        logger.debug(f"API Key length: {len(OPENROUTER_API_KEY)}")
        logger.debug(f"API Key starts with: {OPENROUTER_API_KEY[:15]}...")
        ...
        if response.status_code == 401:
            logger.error(
                "API Key is invalid or expired. Please check your OpenRouter account."
            )
            raise ValueError("OpenRouter API key is invalid or expired")
        ...
        if "401" in str(e):
            raise Exception(
                "Klucz API OpenRouter jest nieprawidłowy lub wygasł. Sprawdź swoje konto na openrouter.ai"
            )
        elif "429" in str(e):
            raise Exception(
            ...
            raise Exception(f"Błąd komunikacji z OpenRouter API: {str(e)}")

    except (KeyError, IndexError, json.JSONDecodeError) as e:
        logger.error(f"Error parsing API response: {str(e)}")
        raise Exception(
            f"Błąd przetwarzania odpowiedzi z OpenRouter API: {str(e)}")
...
def analyze_cv_score(cv_text, job_description="", language='pl'):
    ...
    - Poniżej 50: CV wymagające całkowite przepisania
    ...
    try:
        ...
        if 'linkedin.com' in domain:
            containers = soup.select(
                '.description__text, .show-more-less-html, .jobs-description__content'
            )
            if containers:
            ...
            containers = soup.select(
                '[data-test="section-benefit-expectations-text"], [data-test="section-description-text"]'
            )
            if containers:
                job_text = '\n'.join([
            ...
            containers = soup.select(
                '.offer-description, .offer-content, .description')
            if containers:
                job_text = containers[0].get_text(separator='\n', strip=True)

        if not job_text:
            potential_containers = soup.select(
                '.job-description, .description, .details, article, .job-content, [class*=job], [class*=description], [class*=offer]'
            )
            ...
            if not job_text and soup.body:
                ...
                if len(job_text) > 10000:
                    ...
                    keywords = [
                        ...
                        'skills', 'experience', 'about the job', 'wymagania',
                        'obowiązki', 'kwalifikacje', 'umiejętności',
                        'doświadczenie', 'o pracy'
                    ]
        ...
        job_text = '\n'.join([
            ' '.join(line.split()) for line in job_text.split('\n')
            if line.strip()
        ])
        ...
        logger.debug("Successfully extracted job description from URL")

        if len(job_text) > 4000:
            logger.debug(
                f"Job description is long ({len(job_text)} chars), summarizing with AI"
            )
